...
@NamedQueries({
    ...
    , @NamedQuery(name = "Paqueterias.findActivos", query = "SELECT p FROM Paqueterias p WHERE p.status = 1")
    , @NamedQuery(name = "Paqueterias.findEliminados", query = "SELECT p FROM Paqueterias p WHERE p.status = 0")    
    , @NamedQuery(name = "Paqueterias.findByNombre", query = "SELECT p FROM Paqueterias p WHERE p.nombre = :nombre")
    , @NamedQuery(name = "Paqueterias.findByPrecioEnvio", query = "SELECT p FROM Paqueterias p WHERE p.precioEnvio = :precioEnvio")
...
public class Paqueterias implements Serializable {
    ...
    public Paqueterias(Integer id, String nombre, int precioEnvio, int status) {
        this.id = id;
        this.nombre = nombre;
